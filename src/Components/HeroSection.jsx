		...
		<>
			...
			<AnimatePresence>
				<div className="flex justify-center items-center z-50 h-screen w-[100%]">
					...
					<motion.div
						animate={{ x: moveLeft ? -420 : 0, scale: moveLeft ? 0.7 : 1 }}
						transition={{ ease: "easeInOut", duration: 0.5 }}
						onClick={() => {
							setMoveLeft(true);
						}}
						className="flex justify-center items-center aspect-square w-[50%] sm:w-[28%] border-slate-400 border-8  drop-shadow-lg">
				...
				<motion.div
					className="absolute opacity-80 border-transparent top-0 bottom-0 right-0 left-0 m-auto z-20 w-2 h-2"
					animate={{ y: moveLeft ? 300 : 0, opacity: moveLeft ? 1 : 0 }}
					transition={{
						ease: easeIn,
						scale: { delay: 2 },
						type: "tween",
					...
					<GiCancel
						className=" scale-[2.5] text-yellow-500 "
						onClick={() => {
							setMoveLeft(false);
						}}
					/>
